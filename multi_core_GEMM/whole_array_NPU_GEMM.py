...
def my_matmul(
    ...
    def device_body():
        ...
        def sequence(A, B, C):
            ...
            # Each BD represents:
            # 1) one row tile for A, i.e., m * K, BD ID [0 - 4]
            # 2) one col tile for B, i.e., n * K, BD ID [5 - 9]
            # 3) one out tile for C, i.e., m * n_aie_rows * n, BD ID [10 - 14]
            max_BDs_per_A_B = 5
            ...
            # First, submit the initial BDs for each A, B and C
            for i in range(total_row_tiles):
                ...
                for j in range(total_col_tiles):
                    ...
                    for shim_col in range(n_aie_cols):
                        ...
                        C_array_row_offset = A_row_index * (n_aie_rows * m * N)
                        C_array_col_offset = B_col_index * (n_aie_cols * n)
            ...
            # modulo counter for A, B and C, BD IDs
            mod_BD_ID_cnt = 0

            # Second, reconfigure BDs for A, B and C
            # when you know that each tile is done
            for i in range(total_row_tiles):
                for j in range(total_col_tiles):

                    # There are A, B and C need to be reconfigured when we haven't finished processing the rows
